package PPI::Transform::ReplaceSybase2;
...
sub document {
    ...
    # 1. Find all occurrences of "Mx::Sybase2"
    my $words = $doc->find(sub {
        $_[1]->isa('PPI::Token::Word') && $_[1]->content eq 'Mx::Sybase2'
    });
    ...
    foreach my $word (@$words) {
        ...
        my ($db_config) = $content =~ /database\s*=>\s*\$config->(DB_NAME|DB_name|DB_REP)\b/i;
        if ($content =~ /\bnew\b/ && $db_config) {
            ...
            my $var_name = $db_role eq 'REP' ? '$rep_db' : '$fin_db';
